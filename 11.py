...
for i, row in enumerate(numbers):
    down = i <= (len(numbers) - length)   # True if there is room down
    if down:
        rows = numbers[i:i+length]
    
    for j in range(len(row)):
        forwards = j <= (len(row) - length)
        
        if forwards:
            array = row[j:j+length]
            current = product(array)  # >
            greatest = current if current > greatest else greatest
            
        if down:
            array = [x[j] for x in rows]
            current = product(array) # v
            greatest = current if current > greatest else greatest
            
            if forwards:
                array = [x[j+y] for y, x in enumerate(rows)] # \
                current = product(array)
                greatest = current if current > greatest else greatest
